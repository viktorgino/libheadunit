...
HeadunitBluetoothServer::HeadunitBluetoothServer(QObject* parent)
    : QObject(parent)
    , rfcommServer_(QBluetoothServiceInfo::RfcommProtocol, this)
{
    connect(&rfcommServer_, &QBluetoothServer::newConnection, this, &HeadunitBluetoothServer::onClientConnected);
}

int HeadunitBluetoothServer::start(const Config& config)
{
    m_config = config;
...
void HeadunitBluetoothServer::onClientConnected()
{
    ...
    if (socket != nullptr) {
        ...
        connect(socket, &QBluetoothSocket::readyRead, this, &HeadunitBluetoothServer::readSocket);
...
void HeadunitBluetoothServer::handleWifiInfoRequest(QByteArray& buffer, uint16_t length)
{
...
void HeadunitBluetoothServer::handleWifiSecurityRequest(QByteArray& buffer, uint16_t length)
{
...
void HeadunitBluetoothServer::sendMessage(const google::protobuf::Message& message, uint16_t type)
{
...
void HeadunitBluetoothServer::handleWifiInfoRequestResponse(QByteArray& buffer, uint16_t length)
{
